...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	 ...
	 * */
	// Sokoban algorithm to find shortest path to goal
	while((&pq)->size > 0) {
		n = heap_delete(&pq);
		...
		// Recording any movement done by player
		for(int i = 0; i < SIZE; i++) {
			...
			node_t* newNode;
			player_moved = applyAction(init_data, n, &newNode, a);
			generated_nodes = generated_nodes + 1;
			if(!player_moved || simple_corner_deadlock(init_data, &newNode->state)){
				free_node(init_data, newNode);
				continue;
			}

			// Converting the map to a 1D map
	...
	// Free Memory of HashTable, Explored, priority queue and flatmap
	ht_clear(&hashTable);
